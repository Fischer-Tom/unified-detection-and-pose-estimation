...
# https://github.com/mentian/object-deformnet/blob/master/preprocess/pose_data.py
...
def annotate_camera_train(data_dir):
    ...
    camera_train = open(os.path.join(data_dir, 'train_list_all.txt')).read().splitlines()
    ...
    dirs = list(tqdm(pool.imap(single_process_camera,
                               [(data_dir, im, intrinsics, model_sizes, mug_meta) for im in camera_train]
                               ), total=len(camera_train), desc='Processing CAMERA train data'))
...
def single_process_camera(data):
    data_dir, img_path, intrinsics, model_sizes, mug_meta = data
    ...
    sizes = np.zeros((len(class_ids), 3), dtype=np.float32)
    ...
    gts['size'] = sizes.astype(np.float32)
...
def annotate_real_train(data_dir):
    ...
    real_train = open(os.path.join(data_dir, 'real_train_list_all.txt')).read().splitlines()
    ...
    pool = Pool(processes=32)
    dirs = list(tqdm(pool.imap(single_process_real,
                               [(data_dir, im, intrinsics, scale_factors, model_sizes, mug_meta) for im in real_train]
                               ), total=len(real_train), desc='Processing Real train data'))
    # write valid img list to file
    with open(os.path.join(data_dir, 'real_train_list.txt'), 'w') as f:
        for img_path in dirs:
            if img_path is not None:
                f.write("%s\n" % img_path)

def single_process_real(data):
    """ Process a single image in Real train data. """

    data_dir, img_path, intrinsics, scale_factors, model_sizes, mug_meta = data
    img_full_path = os.path.join(data_dir, img_path)
    ...
    gts['bboxes'] = bboxes  # np.array, [[y1, x1, y2, x2], ...]
    gts['scales'] = scales.astype(np.float32)  # np.array, scale factor from NOCS model to depth observation
    gts['size'] = sizes.astype(np.float32)  # np.array, size in canonical frame
    gts['rotations'] = rotations.astype(np.float32)  # np.array, R
    gts['translations'] = translations.astype(np.float32)  # np.array, T
    gts['instance_ids'] = instance_ids  # int list, start from 1
    gts['model_list'] = model_list  # str list, model id/name
...
def annotate_test_data(data_dir):
    ...
    camera_val = open(os.path.join(data_dir, 'val_list_all.txt')).read().splitlines()
    real_test = open(os.path.join(data_dir, 'real_test_list_all.txt')).read().splitlines()
    ...
    subset_meta = [('CAMERA', camera_val, camera_intrinsics, 'val'), ("Real", real_test, real_intrinsics, 'real_test')]
    for source, img_list, intrinsics, subset in subset_meta:
        pool = Pool(processes=32)
        dirs = list(tqdm(pool.imap(single_process_test,
                                   [(data_dir, im, source, model_sizes, mug_meta) for im in
                                    img_list]
                                   ), total=len(img_list), desc=f'Processing {source} test data'))

        # write valid img list to file
        with open(os.path.join(data_dir, subset+'_list.txt'), 'w') as f:
            for img_path in dirs:
                if img_path is not None:
                    f.write("%s\n" % img_path)

def single_process_test(data):
    data_dir, img_path, source, model_sizes, mug_meta = data
    img_full_path = os.path.join(data_dir, img_path)
    all_exist = os.path.exists(img_full_path + '_color.png') and \
                os.path.exists(img_full_path + '_coord.png') and \
                os.path.exists(img_full_path + '_depth.png') and \
    ...
    nocs_dir = os.path.join(data_dir, 'deformnet_eval', 'nocs_results')
    if source == 'CAMERA':
    ...
    translations = np.zeros((num_insts, 3))
    for i in range(num_insts):
        # Corrupt Model in the NOCS dataset
    ...
    gts['instance_ids'] = instance_ids  # int list, start from 1
    gts['model_list'] = model_list  # str list, model id/name
    gts['size'] = sizes.astype(np.float32)  # 3D size of NOCS model
    gts['scales'] = scales.astype(np.float32)  # np.array, scale factor from NOCS model to depth observation
    gts['rotations'] = rotations.astype(np.float32)  # np.array, R
    gts['translations'] = translations.astype(np.float32)  # np.array, T
...
if __name__ == '__main__':
    args = argparse.ArgumentParser("Preprocess annotations for Real275 dataset")
    args.add_argument('--data_path', type=str, required=True)
    args = args.parse_args()

    # create list for all data
    create_img_list(args.data_path)
    # annotate dataset and re-write valid data to list
    annotate_real_train(args.data_path)
    annotate_camera_train(args.data_path)
    annotate_test_data(args.data_path)
